...
function refreshNews(type) {
    ...
    $.ajax({
        url:'../task8_1/server/getnews.php',
        type:'GET',
        dataType:'json',
        data:{newstype:type},
        success:function (data) {
            data.forEach(function (item) {
                ...
                var indexListMainTitle = $("<div></div>").addClass("index-list-main-title").html(item.newstitle).appendTo(indexListMain);
                ...
                var tipTime = $("<b></b>").addClass("tip-time").html(item.newstime.substring(0,16)).appendTo(indexListBottom);
            });
        },
        error:function () {
