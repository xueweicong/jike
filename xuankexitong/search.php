			  ...
			  if(!empty($_POST["compulsory"])){
			  	  ...
			  	  foreach($compulsory as $value){
					  ...
					  while($row=mysql_fetch_array($result)){
				  	  ...
				  	  while($row=mysql_fetch_array($result)){
					  	  if($cname==$row['CNAME'] or strpos($cname,"俱乐部")!=false and strpos($row['CNAME'],"俱乐部")!=false){
						  	  $same=true;
						  	  break;
					  	  }
				  	  }
        ...
        <div id="navbar" class="navbar-collapse collapse">
          ...
          <form class="navbar-form navbar-right" method="post" action="search.php">
            <select class="form-control" name="order">
              <option value="课程号" selected="selected">课程号</option>
              <option value="课程名"<?php if($order=="课程名"){echo "selected='selected'";}?>>课程名</option>
              <option value="主选教师"<?php if($order=="主选教师"){echo "selected='selected'";}?>>主选教师</option>
              <option value="主选班级"<?php if($order=="主选班级"){echo "selected='selected'";}?>>主选班级</option>
            </select>
            <input type="text" class="form-control"  name="search" placeholder="查找" value="<?php echo $search;?>">
          </form>
        </div>
      </div>
    </nav>
      ...
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          ...
          <div class="table-responsive">
            ...
            <thead>
              <tr>
                <th><h2 class="page-header"><?php echo $order,"“",$search;?>”查找结果</h2>
            ...
            <div class="table-responsive">
              <table class="table table-striped">
                ...
                <?php
			      ...
			      $result=mysql_query("SELECT * FROM COURSE WHERE $char LIKE '%$search%' ORDER BY CNO");
			      while($row=mysql_fetch_array($result)){
					  $i=false;
					  $cno=$row['CNO'];
